...
GPT2_SPECIAL_TOKENS = ["<bos>", "<eos>", "<speaker1>", "<speaker2>","<cap>", "<video>", "<pad>"]
GPT2_SPECIAL_TOKENS_DICT = {'bos_token': "<bos>", 'eos_token': "<eos>", 'additional_special_tokens': ["<speaker1>", "<speaker2>", "<video>", "<cap>"], 'pad_token': "<pad>"}
...
class AVSDDataSet(Dataset):
    def __init__(self, dialogs, tokenizer, features=None, drop_rate=0.5, train=True, model='gpt'):
    ...
    def __getitem__(self, index):
        ...
        type_labels = torch.Tensor(instance["type_labels"]).long()
        ...
        if self.features is not None:
            ...
            return input_ids, token_type_ids, lm_labels, i3d, type_labels
        else:
            return input_ids, token_type_ids, lm_labels, type_labels
...
def collate_fn(batch, pad_token, features=None):
    ...
    input_ids_list, token_type_ids_list, lm_labels_list, i3d_list, type_labels_list = [], [], [], [], []
    for i in batch:
        ...
        if features is not None:
            i3d_list.append(i[3])
            type_labels_list.append(i[4])
        else:
            type_labels_list.append(i[3])
    ...
    reply_mask = torch.zeros(input_mask.size())
    if features is not None:
        i3d = padding(i3d_list, pad_token)
        i3d_mask = torch.sum(i3d != 1, dim=2) != 0
        input_mask = torch.cat([i3d_mask, input_mask], dim=1) # bz, seq_len, feature_dim
        i3d_labels = torch.ones((i3d.size(0), i3d.size(1))).long() * -1 # 
        gpt_video_mask = torch.cat([torch.zeros((i3d.size(0), i3d.size(1))), torch.ones(lm_labels.size())], 1) # video_mask keep language
        reply_mask = torch.zeros(gpt_video_mask.size()) # reply_mask mask none
        lm_labels = torch.cat([i3d_labels, lm_labels], dim=1)
        return input_ids, token_type_ids, lm_labels, input_mask, i3d, gpt_video_mask, reply_mask, type_labels
    else:
        return input_ids, token_type_ids, lm_labels, input_mask, reply_mask, type_labels
